                    ...
                    <div class="toolbar row mb-3">
                                    ...
                                    <div class="mb-2">
                                        <form action="{{ route('docsMon.index') }}" method="GET">
                                            <input type="text" class="form-control" name="search"
                                                placeholder="Search...">
                                            <button type="submit" class="btn btn-sm  btn-outline-primary"><span
                        ...
                        <div class="col ml-auto">
                            <div class="dropdown float-right">
                                <a class="btn btn-primary float-right ml-3" href="{{ route('docsMon.create') }}">Add more
                                    +</a>
                            </div>
                        </div>
                    </div>
                                            ...
                                            <select name="status_id" class="d">
                                                @foreach ($statuses as $status)
                                                    <option value="{{ $status->id }}"
                                                        @if ($status->id == $doc->status_id) selected @endif>
                                                        {{ $status->name }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <td>
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            <a class="dropdown-item"
                                                href="{{ route('docsMon.edit', encrypt($doc->id)) }}">Edit</a>
                                            <form method="POST"
                                                action="{{ route('docsMon.destroy', encrypt($doc->id)) }}">
                                                @csrf
                                                @method('DELETE')
                                                <button class="dropdown-item" type="submit"
                                                    onclick="return confirm('Do You Want Delete?')">Remove</button>
                    ...
                    <nav aria-label="Table Paging" class="mb-0 text-muted">
                        <ul class="pagination justify-content-end">
                            <li class="page-item {{ $currentPage == 1 ? 'disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $currentPage == 1 ? '#' : route('docsMon.index', ['page' => $currentPage - 1]) }}">Previous</a>
                            </li>
                            @for ($i = 1; $i <= $totalPages; $i++)
                                <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                                    <a class="page-link"
                                        href="{{ route('docsMon.index', ['page' => $i]) }}">{{ $i }}</a>
                                </li>
                            @endfor
                            <li class="page-item {{ $currentPage == $totalPages ? 'disabled' : '' }}">
                                <a class="page-link"
                                    href="{{ $currentPage == $totalPages ? '#' : route('docsMon.index', ['page' => $currentPage + 1]) }}">Next</a>
                            </li>
                        </ul>
                    </nav>
