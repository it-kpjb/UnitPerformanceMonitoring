...
@section('content')
    ...
    <div class="row align-items-center mb-4">
        <div class="col">
            <h2 class="h3 mb-0 text-gray-800 font-weight-bold">Manage Roles</h2>
            <p class="text-muted mb-0">View, create, and manage system roles and permissions</p>
        </div>
        <div class="col-auto">
            @can('create-role')
            <a href="{{ route('roles.create') }}" class="btn btn-primary px-4 shadow-sm" style="border-radius: 0.5rem; font-weight: 500;">
                <i class="fe fe-plus mr-1"></i> Add New Role
            </a>
            @endcan
        </div>
    </div>
        ...
        <div class="col-md-12">
            <div class="card shadow-sm border-0" style="border-radius: 0.75rem; overflow: hidden;">
                <div class="card-header bg-white border-bottom-0 py-3 d-flex align-items-center justify-content-between">
                    <h5 class="card-title mb-0 font-weight-bold text-dark" style="font-size: 1.1rem;">Roles List</h5>
                </div>
                        ...
                        <tbody style="border-top: none;">
                            @forelse ($roles as $role)
                                <tr style="border-bottom: 1px solid #f3f4f6;">
                                    <td class="align-middle text-muted" style="padding: 1rem;">{{ $loop->iteration + (method_exists($roles, 'currentPage') ? ($roles->currentPage() - 1) * $roles->perPage() : 0) }}</td>
                                    ...
                                    <td class="align-middle text-right" style="padding: 1rem;">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right shadow-sm border-0" style="border-radius: 0.5rem;">
                                                <a class="dropdown-item text-secondary py-2" href="{{ route('roles.show', $role->id) }}">
                                                    <i class="fe fe-eye mr-2"></i> Show
                                                </a>
                                                
                                                @if ($role->name!='Super Admin')
                                                    @can('edit-role')
                                                        <a class="dropdown-item text-secondary py-2" href="{{ route('roles.edit', $role->id) }}">
                                                            <i class="fe fe-edit-2 mr-2"></i> Edit
                                                        </a>   
                                                    @endcan
                    
                                                    @can('delete-role')
                                                        @if ($role->name != (Auth::user()->hasRole($role->name) ? $role->name : ''))
                                                            <div class="dropdown-divider my-1"></div>
                                                            <form action="{{ route('roles.destroy', $role->id) }}" method="post">
                                                                @csrf
                                                                @method('DELETE')
                                                                <button class="dropdown-item text-danger py-2" type="submit" onclick="return confirm('Do you want to delete this role?');">
                            ...
                            @empty
                                <tr>
                                    <td colspan="3" class="text-center py-5 text-muted">
                                        <div class="mb-3"><i class="fe fe-shield fe-32 text-light"></i></div>
                                        <p class="mb-0">No roles found in the database.</p>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
                ...
                <div class="card-footer bg-white border-top-0 py-3 p-4" style="border-top: 1px solid #f3f4f6 !important;">
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        <p class="text-muted mb-0" style="font-size: 0.85rem;">
                            Showing <span class="font-weight-bold text-dark">{{ $roles->firstItem() ?? 0 }}</span> to <span class="font-weight-bold text-dark">{{ $roles->lastItem() ?? 0 }}</span> of <span class="font-weight-bold text-dark">{{ $roles->total() }}</span> roles
                        </p>
